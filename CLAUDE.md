...
- Run a scene: `go run . <command>`, e.g. `go run . scene` or `go run . clock` (outputs a `.ppm`/`.png` to the repo root); override the destination with the persistent `--output <file>` flag — a `.ppm` extension (case-insensitive) selects PPM, anything else PNG.
...
- `pkg/objects` — `Ray`, `Sphere`, intersection logic, and the single `Object` interface (`Intersect`/`NormalAt`/`Material`) consumed by `scene.World` and `scene.Computations`; depends on `pkg/shader` because `Object.Material()` returns `*shader.Material`.
